...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasTable('taxonomy_versions')) {
            Schema::create('taxonomy_versions', function (Blueprint $table): void {
                ...
                $table->index(['status', 'published_at'], 'tax_ver_status_published_idx');
            });
        }

        if (! Schema::hasTable('taxonomy_mapping_suggestions')) {
            Schema::create('taxonomy_mapping_suggestions', function (Blueprint $table): void {
                ...
                $table->index(['merchant_id', 'merchant_company_id', 'status'], 'tax_map_merchant_status_idx');
                $table->index(['suggestion_type', 'confidence_level'], 'tax_map_type_conf_idx');
                $table->index(['merchant_category_id', 'status'], 'tax_map_category_status_idx');
                $table->index(['merchant_brand_id', 'status'], 'tax_map_brand_status_idx');
            });
        }

        if (! Schema::hasTable('taxonomy_learning_events')) {
            Schema::create('taxonomy_learning_events', function (Blueprint $table): void {
                ...
                $table->index(['merchant_id', 'merchant_company_id', 'event_type'], 'tax_learn_merchant_event_idx');
                $table->index(['target_type', 'created_at'], 'tax_learn_target_created_idx');
            });
        }

        DB::table('taxonomy_versions')->updateOrInsert(
