...
- Em telas dos portais, evitar feedback inline para ações operacionais; usar modal central para sucesso, erro e orientação, especialmente em integrações, importações e validações.
- Tooltips de formulário devem ficar contidos no viewport e não depender de `title` nativo quando o texto puder sair da tela.
