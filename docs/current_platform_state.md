...
- Migração assistida Sizebay: `/api/v1/imports/preview`, `/api/v1/imports`, `/api/v1/imports/{id}` e `/api/v1/imports/{id}/rollback` agora aceitam `type=sizebay_migration` com pacote `JSON`, `CSV`, `XLSX` ou `ZIP`, seção isolada para arquivos únicos, comparação opcional com BigShop, resumo por seção, cobertura, fila de revisão, `batch_id`, rollback e bloqueio explícito para dados identificáveis ou segredos em relatórios.
- Portal de importações: `/app/importacoes` agora cobre importação simples de produtos/tabelas e também a migração assistida de clientes vindos da Sizebay, com upload, prévia, cobertura, pendências revisáveis, histórico detalhado do lote e desfazer do lote.
- Sprint 160 foi publicada em produção no commit `4cd0d2f`, com o run `26678665133` verde; a validação pós-deploy confirmou `/app/importacoes`, APIs protegidas, widget, SaaS, portal e `PRODUCTION VALIDATION OK`.
...
Retestar uma contratação Pix real em produção e acompanhar webhook/cron; depois executar cartão de baixo valor, validar recorrência anual com a operadora e usar o novo fluxo de migração assistida da Sprint 160 com material autorizado de cliente real para consolidar a operação de saída da Sizebay.
