...
- Widget público: `/widget/v1/provador-virtual.js` e `/widget/v1/provador-virtual.css`. O tema aceita `button_style`, `button_background` e `button_text` para variar os botões públicos entre visual com brilho, minimal com ícones, contorno leve e pílulas suaves, preservando os botões `PV Descubra seu tamanho` e `cm Tabela de Medidas`.
